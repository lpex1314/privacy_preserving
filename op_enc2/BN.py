...
class BatchNormFunction(torch.autograd.Function):
    ...
    def forward(ctx, y_pre: Tensor, labels: Tensor, classes: int) -> Tensor:
        ...
        shape_fr, delta1_c, len_r, float_array_del1 = preprocess(delta_dec)
        ...
        ctx.save_for_backward(loss, y_pre, labels, delta_dec)
        ctx.classes = classes
...
class BatchNorm(nn.Module):
    ...
    def forward(self, input: Tensor, training: bool) -> Tensor:
        return BatchNormFunction.apply(input, training)
