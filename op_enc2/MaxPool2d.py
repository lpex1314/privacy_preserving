...
class MaxPool2dFunction(torch.autograd.Function):
    ...
    def forward(ctx, input, kernel_size, stride):
        ...
        N, C, H, W = shape_x[0] // 10, shape_x[1], shape_x[3], shape_x[3]
        ...
        len_x = N * C * H * W
        res = torch.rand([10*len_out], dtype=torch.double)
        shape_res, res_c, len_res, float_array_res = preprocess(res)
        len_x_c = c_int(len_x)
        len_out_c = c_int(len_out)
        shape_new = torch.Size([N * 10, C, H_out, W_out])
    ...
    def backward(ctx, gradOutput):
        ...
        shape_x = input.shape
        len_x = input.flatten().shape[0] // 10
        len_y = len_y // 10
        len_x_c = c_int(len_x)
        len_y_c = c_int(len_y)
