...
class MaxPool2dFunction(torch.autograd.Function):
    ...
    def forward(ctx, input, kernel_size, stride):
        ...
        shape_fr, delta1_c, len_r, float_array_delta1 = preprocess(delta_dec)
        ...
        shape_new = torch.Size([N, C, H_out, W_out])
        delta_enc = delta_enc.resize_(encipher.N_ks, *shape_new)
        delta_list.set_delta(delta_enc)
        shape_delta2, delta2_c, len_del, float_array_delta2 = preprocess(delta_enc)
        ...
        ctx.save_for_backward(input, result, arg_out, delta_dec, delta_enc)
        ctx.H_out = H_out
        ctx.W_out = W_out
        ctx.stride = stride
